...
- `MainPage.vue` — The cockpit: both eye views rendered side-by-side and **swapped** for cross-eyed free-viewing (right eye on screen-left), a size slider (bigger previews are harder to cross-eye-fuse), mouse-look + WASD/Space/Shift for head 6DoF, and keyboard-driven controllers (right: arrows/RCtrl/RShift/7-8-9-0, left: IJKL/R/F/1-2-3-4 — see the in-page help icon for the full legend). `q-page` only gets a `min-height` from Quasar in this app, not a real height, so the page measures the actual available height in JS (`availableHeight`/`updateAvailableHeight`) rather than relying on flex-grow-fills-viewport, which silently doesn't work here.
